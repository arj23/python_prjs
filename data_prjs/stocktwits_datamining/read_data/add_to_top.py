...
def get_messages_from_stocktwits(token,last_message_id=None):
    ...
    if last_message_id is None:
        api_url = 'https://api.stocktwits.com/api/2/streams/all.json?access_token={}&since'.format(token)
    else:
        api_url = 'https://api.stocktwits.com/api/2/streams/all.json?access_token={}&since={}'.format(token,last_message_id)
    try:
...
def main() :
    ...
    while (message_count < max_count):
        if max_id is 0:
            ...
            result = msgs_collection.find().sort("id", pymongo.DESCENDING).limit(1);
            for r in result:
                max_id = r['id']
        messages = get_messages_from_stocktwits(token_list[token_list_id], max_id)
